...
const defaultEmployee: Omit<Employee, 'id'> = {
  firstName: '', lastName: '', email: '', phone: '', department: '', role: '', dateOfJoining: '', status: 'active',
};
...
const EmployeesPage: React.FC = () => {
  ...
  const filtered = employees
    .filter(e => `${e.firstName} ${e.lastName} ${e.email} ${e.department}`.toLowerCase().includes(search.toLowerCase()))
    .sort((a, b) => {
  ...
  const handleSave = () => {
    ...
    if (editing) {
      ...
      setEmployees(prev => [...prev, { ...form, id: Date.now().toString() } as Employee]);
